// Thin client-side wrapper: actual Gemini call happens on the server so the
...
export const generateInfographicImage = async (
  topic: string,
  _apiKey: string, // ignored; server reads from env.
  colors: string[],
  fontFamily: string,
  ...
  meta?: { variation?: 'baseline' | 'tuned' | 'reimagined'; visualRhetoric?: string; sourceName?: string; referenceImageBase64?: string | null; projectId?: string | null },
  signal?: AbortSignal,
): Promise<string> => {
